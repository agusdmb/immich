<script lang="ts">
  ...
  const handleFavorite = async () => {
    try {
      if (isLiked) {
        ...
        activityManager.activities = activityManager.activities.filter((reaction) => reaction.id !== activityId);
        isLiked = null;
      } else {
        isLiked = await createActivity({
          activityCreateDto: { albumId: album.id, type: ReactionType.Like },
        });
        activityManager.activities = [...activityManager.activities, isLiked];
      }
    } catch (error) {
  ...
  const updateComments = async () => {
    try {
      await activityManager.updateActivities(album.id);
    } catch (error) {
